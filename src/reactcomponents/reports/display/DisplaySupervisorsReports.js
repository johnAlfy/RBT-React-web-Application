...
import AddBus from "../../routes/bus/AddBus";
import AddReport from "../add/AddReport";

class DisplaySupervisorsReports extends Component {
    constructor(props) {
        super(props);
        ...
        axios({
            url: '/review_reports_supervisors',
            method: 'get',
        }).then((res) => {
            if (res.data.reports) {
                this.setState({
    ...
    onInputSearchChange = (e) => {
        ...
        else if (searchOption === "supervisorId")
            newReports = this.state.reports.filter(report => report.supervisor.username.includes(value));
                ...
                <Row>
                    ...
                    <Col xs={{size: 3}}>
                        <Input type="select" name="select" id="searchOption" onChange={this.selectSearch}>
                            ...
                            <option id={'supervisorId'} value={'supervisorId'}>Supervisor Name</option>
    ...
    render() {
        return (
            <div>
                <Col>
                    <Button id="toggler_addSupReport" style={{marginBottom: '1rem',backgroundColor: '#271670'}}>
                        Notify Supervisors
                    </Button>
                    <UncontrolledCollapse toggler="#toggler_addSupReport">
                        ...
                        this.state.searchReports.map((report, index) => {
                            return (
                                <tr key = {report.id}>
                                    ...
                                    <td>{report.supervisor.firstName} {report.supervisor.lastName}</td>
...
export default DisplaySupervisorsReports;